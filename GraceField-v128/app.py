...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.28', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
def gerar_relatorio_pdf(result_df=None, horiz_df=None, declarantes=None, periodo='—', decimos=None, dedup_stats=None):
    """Gera o relatório técnico em A4 paisagem.

    Ordem do relatório:
      1. Identificação dos autos / declarantes / critérios
      2. Demonstração horizontal
      3. Resultado consolidado (última seção do relatório)

    A demonstração horizontal identifica cada vínculo pelo CNPJ e usa duas
    colunas fixas por vínculo: Remuneração e Previdência.
    """
    buf = BytesIO()
    land = landscape(A4)
    doc = BaseDocTemplate(
    ...
    h1 = ParagraphStyle(
        ...
        fontSize=16, leading=18, textColor=colors.HexColor('#17365D'),
        spaceBefore=6, spaceAfter=6,
    )
    body = ParagraphStyle(
        'JFBODY', parent=styles['BodyText'], fontSize=11.5, leading=14,
        textColor=colors.HexColor('#344054'),
    )
    small = ParagraphStyle(
        'JFS', parent=body, fontSize=10.5, leading=13,
        textColor=colors.HexColor('#475467'),
    )
    th = ParagraphStyle(
        'JFTH', parent=styles['BodyText'], fontName='Helvetica-Bold',
        fontSize=10.5, leading=12, textColor=colors.HexColor('#17365D'),
        alignment=TA_CENTER,
    )
    th_white = ParagraphStyle(
        'JFTHW', parent=th, textColor=colors.white, fontSize=11, leading=12.5,
    )
    # Somente a célula "Competência" recebe uma redução de um ponto para
    # evitar qualquer estouro do texto na primeira coluna da demonstração.
    th_comp = ParagraphStyle(
        'JFTHCOMP', parent=th_white, fontSize=11, leading=12.5,
        alignment=TA_CENTER,
    )
    ...
    ti = _pdf_table(ident, [52 * mm, 215 * mm], 9.2, header=False)
    ...
    for i, d in enumerate(declarantes or [], 1):
        rows.append([
            str(i), _pdf_text(d.get('nome', '')), _pdf_text(d.get('cnpj', '')),
            _pdf_text(d.get('grupo', '')), _pdf_text(d.get('paginas', '')),
        ])
    if len(rows) == 1:
        rows.append(['—', 'Nenhum declarante selecionado', '—', '—', '—'])
    story.append(_pdf_table(rows, [10 * mm, 92 * mm, 48 * mm, 38 * mm, 79 * mm], 9.2))
    ...
    if horiz_df is not None and not horiz_df.empty:
        ...
        story.append(Paragraph(
            ...
            'Cada CNPJ ocupa um par fixo de colunas: Remuneração e Previdência.', body
        ))
        ...
        for chunk_idx in range(0, len(pares), chunk_size):
            ...
            for j, (cnpj, _, _) in enumerate(chunk, chunk_idx + 1):
                ...
                rel.append([
                    str(j), _pdf_text(d.get('nome', '') or 'Declarante não identificado'),
                    _pdf_text(cnpj), _pdf_text(d.get('grupo', '') or '—'),
                    _pdf_text(d.get('paginas', '') or '—'),
                ])
            story += [
                Paragraph('Declarantes deste bloco', body),
                _pdf_table(rel, [10 * mm, 92 * mm, 48 * mm, 38 * mm, 79 * mm], 9.0),
                Spacer(1, 5),
            ]

            # Cabeçalho em dois níveis. "Competência" ocupa uma célula própria.
            head1 = [Paragraph('Competência', th_comp)]
            head2 = [Paragraph('', th)]
            for cnpj, _, _ in chunk:
                head1.append(Paragraph(_pdf_text(cnpj), th_white))
                head1.append('')
                head2.extend([Paragraph('Remuneração', th), Paragraph('Previdência', th)])
            data = [head1, head2]

            for _, r in horiz_df.iterrows():
                # Nunca usar iloc/valor formatado de forma que a competência
                # possa desaparecer. O primeiro campo é sempre o rótulo mensal.
                comp_raw = r.get('Competência', '')
                comp_label = str(comp_raw).strip() if comp_raw is not None else ''
                if not comp_label or comp_label.lower() in {'nan', 'none'}:
                    comp_label = ''
                row = [comp_label]
            ...
            # Corpo confortável; a única redução específica é o cabeçalho
            # "Competência", que usa 11 pt para garantir encaixe.
            table_font = 10.5 if ncols <= 9 else 9.8
    ...
    # ------------------------------------------------------------------
    # 4. RESULTADO CONSOLIDADO — SEM SEÇÕES POSTERIORES
    # ------------------------------------------------------------------
    ...
    if result_df is not None and not result_df.empty:
        ...
        for _, r in result_df.iterrows():
            # A competência já deve vir pronta da camada de apresentação.
            # O fallback abaixo cobre DataFrames técnicos com competência
            # ainda em YYYY-MM.
            comp = r.get('Competência', '')
            if not comp or str(comp).strip().lower() in {'nan', 'none'}:
                raw = r.get('competencia', '')
                tipo_raw = str(r.get('tipo_competencia', 'mensal'))
                comp = competencia_br(str(raw), tipo_raw) if raw is not None else ''
            row = [
                Paragraph(_pdf_text(comp), th),
                Paragraph(_pdf_text(r.get('Tipo', '')), th),
                *[_pdf_money(r.get(c, 0)) for c in cols[2:]],
            ]
            data.append(row)
        widths = [29 * mm, 21 * mm, 30 * mm, 29 * mm, 33 * mm, 31 * mm, 31 * mm, 32 * mm, 33 * mm]
        story.append(_pdf_table(data, widths, 11.0, align_right_cols=list(range(2, 9))))
        tr = float(pd.to_numeric(result_df['Recolhido'], errors='coerce').fillna(0).sum()) if 'Recolhido' in result_df else 0
        tm = float(pd.to_numeric(result_df['Máx. total'], errors='coerce').fillna(0).sum()) if 'Máx. total' in result_df else 0
        te = float(pd.to_numeric(result_df['Acima do teto'], errors='coerce').fillna(0).sum()) if 'Acima do teto' in result_df else 0
        story += [
            Spacer(1, 6),
            _pdf_table([
                ['Total das contribuições recolhidas', 'Total máximo considerado', 'Total acima do teto'],
                [_pdf_money(tr, True), _pdf_money(tm, True), _pdf_money(te, True)],
            ], [89 * mm, 89 * mm, 89 * mm], 11.0),
        ]
    else:
        story.append(_pdf_table(
    ...
    # Deliberadamente não há nenhuma seção depois do Resultado consolidado.
    doc.build(story)
    return buf.getvalue()
    ...
    if ap_base.empty:
        ...
        if ap_scope.empty:
            ...
            if result.empty:
                ...
                st.download_button('Baixar apuração CSV', csv_bytes(result), 'apuracao_previdenciaria_v1_2_28.csv', 'text/csv')
