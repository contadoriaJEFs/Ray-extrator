...
display["competencia_br"] = display.apply(
    lambda r: competencia_br(r["competencia"], r["tipo_competencia"]), axis=1
)
display["competencia_iso"] = display["competencia"]
display["competencia"] = display["competencia_br"]

view_cols = [
    "ano_calendario", "competencia", "cnpj_declarante",
    "nome_declarante_dirf", "codigo_receita",
    "rendimento_tributavel", "irrf", "previdencia_oficial",
    "tipo_competencia", "pagina_pdf"
]
view = display[view_cols].copy()
...
# JSON: sempre completo, para servir de base ao futuro motor previdenciário.
json_payload = {
...
# Base exportável em formato amigável para planilha.
export_df = df.copy()
...
essential_cols = [
    "ano_calendario", "cnpj_declarante", "nome_declarante_dirf",
    "codigo_receita", "competencia_br", "tipo_competencia",
    "rendimento_tributavel", "irrf", "previdencia_oficial",
    "pagina_pdf", "arquivo_origem",
]
all_cols = list(export_df.columns)

st.markdown("**CSV personalizado**")
st.caption("O JSON permanece completo. No CSV você escolhe exatamente as colunas que deseja levar para a planilha.")

preset = st.radio(
    "Modelo de colunas",
    ["Essenciais previdenciárias", "Todas as colunas", "Personalizado"],
    horizontal=True,
)
...
if preset == "Essenciais previdenciárias":
    ...
    selected_cols = st.multiselect(
        "Marque as colunas para o CSV",
        options=all_cols,
        default=[c for c in essential_cols if c in export_df.columns],
    )

if selected_cols:
    csv_df = export_df[selected_cols].copy()
    csv_labels = {
        "ano_calendario": "Ano", "cnpj_declarante": "CNPJ Declarante",
        "nome_declarante_dirf": "Declarante", "codigo_receita": "Código Receita",
        "descricao_codigo_receita": "Descrição Código Receita",
        "competencia_br": "Competência", "competencia": "Competência ISO",
        "tipo_competencia": "Tipo", "rendimento_tributavel": "Rendimento Tributável",
        "irrf": "Imposto Retido", "previdencia_oficial": "Previdência Oficial",
        "dependentes": "Dependentes", "pensao_alimenticia": "Pensão Alimentícia",
        "desconto_simplificado": "Desconto Simplificado",
        "previdencia_complementar": "Previdência Complementar",
        "compensacao_judicial_ano_calendario": "Compensação Judicial Ano-Calendário",
        "compensacao_judicial_anos_anteriores": "Compensação Judicial Anos Anteriores",
        "fundo_clube": "Fundo/Clube", "numero_processo": "Número do Processo",
        "pagina_pdf": "Página PDF", "arquivo_origem": "Arquivo Origem",
    }
    csv_df = csv_df.rename(columns={k: v for k, v in csv_labels.items() if k in csv_df.columns})
    csv_data = csv_bytes(csv_df)
    st.dataframe(csv_df.head(30), use_container_width=True, hide_index=True)
    st.caption(f"{len(selected_cols)} coluna(s) selecionada(s) para o CSV.")

    xlsx_buffer = io.BytesIO()
    with pd.ExcelWriter(xlsx_buffer, engine="openpyxl") as writer:
        csv_df.to_excel(writer, index=False, sheet_name="Exportacao")
        if checks:
    ...
    b1.download_button("Baixar JSON completo", json_data, "extracao_dirf_completa.json", "application/json")
    b2.download_button("Baixar CSV selecionado", csv_data, "extracao_dirf_selecionado.csv", "text/csv")
    b3.download_button(
        "Baixar Excel", xlsx_buffer.getvalue(), "extracao_dirf.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
else:
    st.warning("Selecione pelo menos uma coluna para gerar o CSV.")

with st.expander("Registro bruto / auditoria"):
    st.write("O registro bruto mantém todos os campos extraídos, incluindo página, arquivo, declarante, código de receita e valores. Esses dados também permanecem no JSON.")
