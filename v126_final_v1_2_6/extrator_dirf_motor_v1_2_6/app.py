...
st.title('Extrator DIRF + Motor Previdenciário V1.2.6')
...
with tabs[3]:
    ...
    st.caption('Bancada independente para validar a tabela histórica contra os valores reais da DIRF. A navegação pode abranger um ano, um intervalo ou todo o histórico disponível; o cálculo permanece mensal, competência a competência.')

    with st.expander('ⓘ Como funciona esta verificação', expanded=True):
        st.markdown("""
**Selecione a fonte e o período do histórico.** O sistema carrega automaticamente as competências mensais encontradas para aquela fonte dentro do período escolhido.
...
""")
    ...
    if mensal_v.empty:
        ...
        if not fonte_map:
            ...
            if sub_hist.empty:
                st.info('Não foram encontrados registros mensais dessa fonte no período selecionado.')
            else:
                ...
                agrupado = sub_hist.groupby(['competencia', 'ano_calendario'], as_index=False).agg(
                ...
                st.caption(f'CNPJ {cnpj_v} · período {periodo_label} · {len(resultados_v)} competência(s) encontrada(s)')
                m1, m2, m3, m4 = st.columns(4)
                ...
                for rr in resumo_rows:
                    copy_rows_v.append([
                        str(rr['Competência'] or ''), str(rr['Metodologia'] or ''), str(rr['Tabela'] or ''),
