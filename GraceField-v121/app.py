...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.21', page_icon='⚖️', layout='wide')
...
def extrair_dados_caso(registros):
    # Obtém somente dados de identificação já presentes no RAW.
    if not registros:
...
st.title('Extrator DIRF + Motor Previdenciário V1.2.21')
...
# Identificação dos autos. Processo e autor são preenchidos automaticamente
# quando já constam da DIRF; o réu permanece para preenchimento manual porque
# a DIRF não oferece esse campo como dado confiável.
caso_extraido = extrair_dados_caso(records)
if 'caso_processo' not in st.session_state:
    st.session_state.caso_processo = metadados_carregados.get('processo') or caso_extraido['processo']
if 'caso_autor' not in st.session_state:
    st.session_state.caso_autor = metadados_carregados.get('autor') or caso_extraido['autor']
if 'caso_reu' not in st.session_state:
    st.session_state.caso_reu = metadados_carregados.get('reu') or ''
if 'caso_documento_id' not in st.session_state:
    st.session_state.caso_documento_id = metadados_carregados.get('id_documento') or ''
