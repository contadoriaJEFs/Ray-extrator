...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.25', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
def gerar_relatorio_pdf(result_df=None, horiz_df=None, declarantes=None, periodo='—', decimos=None, dedup_stats=None):
    ...
    title = ParagraphStyle(
        'JFT', parent=styles['Title'], fontName='Helvetica-Bold', fontSize=15,
        leading=18, textColor=colors.HexColor('#17365D'), alignment=TA_CENTER,
        spaceAfter=5,
    )
    sub = ParagraphStyle(
        'JFSUB', parent=styles['BodyText'], fontName='Helvetica-Bold',
        fontSize=9.5, leading=11, textColor=colors.HexColor('#475467'),
    ...
    small = ParagraphStyle(
        'JFS', parent=styles['BodyText'], fontSize=7.2, leading=9.2,
        textColor=colors.HexColor('#475467'),
    )
    th = ParagraphStyle(
        'JFTH', parent=styles['BodyText'], fontName='Helvetica-Bold',
        fontSize=6.7, leading=7.8, textColor=colors.HexColor('#17365D'),
        alignment=TA_CENTER,
    )
    th_white = ParagraphStyle(
        'JFTHW', parent=th, textColor=colors.white,
    )
    ...
    ti = _pdf_table(ident, [52 * mm, 215 * mm], 8.0, header=False)
    ...
    story.append(_pdf_table(rows, [10 * mm, 92 * mm, 48 * mm, 38 * mm, 79 * mm], 7.0))
    ...
    story += [PageBreak(), Paragraph('3. Resultado consolidado', h1)]
    if result_df is not None and not result_df.empty:
        cols = [
            'Competência', 'Tipo', 'Recolhido', 'Teto', 'Máx. progressiva',
            'Máx. 11%', 'Máx. 20%', 'Máx. total', 'Acima do teto', 'Status'
        ]
        data = [[Paragraph(_pdf_text(c), th) for c in cols]]
        for _, r in result_df.iterrows():
            data.append([
                _pdf_text(r.get('Competência', '')),
                _pdf_text(r.get('Tipo', '')),
                *[_pdf_money(r.get(c, 0)) for c in cols[2:9]],
                _pdf_text(r.get('Status', '')),
            ])
        widths = [24 * mm, 15 * mm, 25 * mm, 24 * mm, 29 * mm, 25 * mm, 25 * mm, 27 * mm, 27 * mm, 46 * mm]
        story.append(_pdf_table(data, widths, 5.9, align_right_cols=list(range(2, 9))))
    ...
    if horiz_df is not None and not horiz_df.empty:
        story += [PageBreak(), Paragraph('4. Demonstração horizontal', h1)]
        ...
        for chunk_idx in range(0, len(pares), chunk_size):
            ...
            if chunk_idx:
                ...
                story.append(Paragraph('4. Demonstração horizontal — continuação', h1))
            ...
            story += [Paragraph('Declarantes deste bloco', small), _pdf_table(rel, [10 * mm, 92 * mm, 48 * mm, 38 * mm, 79 * mm], 6.7), Spacer(1, 4)]

            # Cabeçalho agrupado em dois níveis.
            head1 = [Paragraph('Competência', th_white)]
            head2 = ['']
            ...
            font = max(6.0, min(7.3, 255 / ncols))
