...
**Documento-base para discussão com a equipe e validação por especialista antes de qualquer nova alteração no código.**

**Versão:** proposta de consolidação do projeto atual  
**Status:** planejamento — nenhum novo código deve ser gerado sem validação prévia.

---

# 1. Objetivo geral do sistema

O sistema está sendo desenvolvido para transformar informações existentes em arquivos **DIRF** em uma apuração previdenciária capaz de responder:

> **Durante determinado período, quanto o segurado efetivamente recolheu de contribuição previdenciária e quanto desse valor ultrapassou o limite máximo de contribuição previdenciária aplicável em cada competência?**
...
O sistema lê o PDF e percorre suas páginas, procurando estruturar as informações da DIRF em registros.
...
Esses números representam o arquivo testado e **não constituem limite técnico definitivo**.
...
> **135 páginas é a quantidade efetivamente testada, não o limite declarado do sistema.**
...
O sistema tenta distinguir fontes potencialmente previdenciárias de fontes claramente não previdenciárias.
...
Quando a natureza é claramente incompatível com o objetivo previdenciário, essas fontes são desmarcadas por padrão.
...
podem ser sugeridas como não pertencentes ao fluxo previdenciário principal.
...
Portanto, a unidade de decisão deve ser a **competência**, não apenas o ano.
...
A interface permite trabalhar com até três casas decimais quando necessário.
...
A comparação é realizada utilizando a diferença interna antes do arredondamento visual.
...
O sistema não deverá calcular cada empresa isoladamente quando a regra exigir consideração conjunta.
...
O tratamento deverá considerar as regras aplicáveis à remuneração conjunta.
...
para utilização da categoria de 20%, conforme a regra adotada pelo motor.
...
> **Qual é a contribuição previdenciária máxima aplicável àquela competência?**
...
Deverá representar o processamento completo, incluindo, conforme a arquitetura final:
...
A planilha de demonstração utilizada no projeto deverá servir como referência para comparar:
...
Remuneração de 11% e remuneração de 20%, com utilização do espaço restante do teto.
...
Remuneração de 11% acima do teto e contribuição efetiva superior ao máximo.
...
Antes do motor ser considerado definitivo, deverá ocorrer validação especializada.
...
O código DIRF não deve ser tratado automaticamente como uma classificação previdenciária legal completa.
...
| 7 | Classificação 11/20/Progressiva | 🟡 Funcional/manual |
| 8 | Classificação automática | 🔜 Próxima etapa |
| 9 | Tabelas históricas | 🟡 Funcional |
| 10 | Verificação mensal | 🟡 Funcional |
| 11 | Detalhamento por competência | ✅ Implementado |
| 12 | Tolerância | ✅ Implementada |
| 13 | Validação progressiva | 🟡 Em validação |
| 14 | Múltiplos vínculos | 🔜 Próxima grande etapa |
| 15 | Motor 11%/20% definitivo | 🟡 Parcial |
| 16 | Motor progressivo definitivo | 🔜 Após validação |
| 17 | Excesso por competência | 🔜 |
| 18 | Excesso por período | 🔜 |
| 19 | Demonstração horizontal | 🟡 Funcional |
| 20 | Exportação | 🟡 Funcional |
| 21 | Memória de cálculo | 🔜 |
| 22 | Auditoria completa | 🔜 |
| 23 | Testes automatizados completos | 🔜 |
| 24 | Validação por especialista | 🔜 |
| 25 | Motor definitivo | 🔜 |
| 26 | Sistema final | 🔜 |

# 81. Prioridade antes de escrever qualquer novo código

Neste momento, o recomendado é **não iniciar uma nova versão do código**.
...
> **“Dada a informação existente na DIRF, qual é exatamente a regra jurídica e matemática que determina, competência por competência, o valor máximo de contribuição previdenciária que poderia ter sido recolhido pelo segurado, considerando todas as fontes e categorias aplicáveis?”**

A partir dessa resposta, o software poderá transformar a regra em cálculo.
...
O principal trabalho restante para chegar ao sistema definitivo está concentrado em:
...
> **O sistema deverá permitir que o usuário envie a DIRF, informe o período que deseja analisar e, após apenas as confirmações realmente necessárias, obtenha uma demonstração auditável de quanto foi efetivamente recolhido, quanto poderia ter sido recolhido segundo a regra previdenciária aplicável a cada competência e qual foi o valor total pago acima do limite previdenciário no período analisado.**

---

## DECISÃO DE PROJETO

**Este documento deve ser tratado como o roadmap funcional de referência antes da próxima etapa de programação.**
