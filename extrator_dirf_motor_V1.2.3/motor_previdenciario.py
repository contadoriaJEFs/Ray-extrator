...
def verificar_tabela(competencia: str, remuneracao: float, previdencia_dirf: Optional[float] = None):
    ...
    diferenca = None if informado is None else round(informado - calc['contribuicao_calculada'], 2)
    if informado is None:
        status = 'SEM_COMPARACAO'
    elif math.isclose(informado, calc['contribuicao_calculada'], abs_tol=0.01):
        status = 'COMPATIVEL'
    else:
