...
st.title('Extrator DIRF + Motor Previdenciário V1.2.3')
...
''')
        ...
        if not fonte_map:
            ...
            if sub_ano.empty:
                ...
                for _, row in sub_ano.iterrows():
                    ...
                    resultados_v.append(verificar_tabela(comp, rem, prev))
    ...
    with st.expander('📚 Tabela histórica cadastrada — 2017 a 2026'):
        ...
        for t in TABELAS_HISTORICAS:
            historico.append({'Vigência':f"{t['inicio']} a {t['fim']}",'Metodologia':t['metodologia'],'Faixas':' · '.join([f"{x[2]*100:g}% até {fmt(x[1])}" for x in t['faixas']]),'Teto':t['teto'],'Fonte':t['fonte']})
