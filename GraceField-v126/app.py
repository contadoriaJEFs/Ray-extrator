...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.26', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
def gerar_relatorio_pdf(result_df=None, horiz_df=None, declarantes=None, periodo='—', decimos=None, dedup_stats=None):
    ...
    title = ParagraphStyle(
        'JFT', parent=styles['Title'], fontName='Helvetica-Bold', fontSize=18,
        leading=21, textColor=colors.HexColor('#17365D'), alignment=TA_CENTER,
        spaceAfter=5,
    )
    sub = ParagraphStyle(
        'JFSUB', parent=styles['BodyText'], fontName='Helvetica-Bold',
        fontSize=11, leading=13, textColor=colors.HexColor('#475467'),
        alignment=TA_CENTER, spaceAfter=5,
    )
    h1 = ParagraphStyle(
        'JFH1', parent=styles['Heading2'], fontName='Helvetica-Bold',
        fontSize=13, leading=15, textColor=colors.HexColor('#17365D'),
        spaceBefore=6, spaceAfter=5,
    )
    small = ParagraphStyle(
        'JFS', parent=styles['BodyText'], fontSize=8.4, leading=10.4,
        textColor=colors.HexColor('#475467'),
    )
    th = ParagraphStyle(
        'JFTH', parent=styles['BodyText'], fontName='Helvetica-Bold',
        fontSize=8.0, leading=9.2, textColor=colors.HexColor('#17365D'),
        alignment=TA_CENTER,
    )
    th_white = ParagraphStyle(
        'JFTHW', parent=th, textColor=colors.white, fontSize=8.0, leading=9.2,
    )
    th_comp = ParagraphStyle(
        'JFTHCOMP', parent=th_white, fontSize=8.2, leading=9.5,
    )
    ...
    if result_df is not None and not result_df.empty:
        ...
        story.append(_pdf_table(data, widths, 7.4, align_right_cols=list(range(2, 9))))
    ...
    if horiz_df is not None and not horiz_df.empty:
        ...
        for chunk_idx in range(0, len(pares), chunk_size):
            ...
            font = max(7.0, min(8.0, 255 / ncols + 0.8))
