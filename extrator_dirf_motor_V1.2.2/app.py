...
st.title('Extrator DIRF + Motor Previdenciário V1.2.2')
st.caption('Extração preservada + seleção inteligente de fontes + agrupamento de vínculos/declarantes + classificação separada + apuração por competência.')
...
''')
    ...
    fonte_map = {'Manual / total da competência': None}
    for v in fontes:
        fonte_map[f"{v['nome']} — CNPJ {v['cnpj']}"] = v['cnpj']
    fonte_label = c2.selectbox('Fonte', list(fonte_map.keys()), key='verif_fonte')
    ...
    rem_v = c3.number_input('Remuneração / salário de contribuição', min_value=0.0, value=default_rem, step=100.0, format='%.2f', key='verif_rem_' + re.sub(r'\W+', '_', comp_v + '_' + (cnpj_v or 'total')))
    comparar_v = c4.checkbox('Comparar com Previdência da DIRF', value=not sub_v.empty, key='verif_compare')
    prev_v = c5.number_input('Previdência informada na DIRF', min_value=0.0, value=default_prev, step=10.0, format='%.2f', disabled=not comparar_v, key='verif_prev_' + re.sub(r'\W+', '_', comp_v + '_' + (cnpj_v or 'total')))
