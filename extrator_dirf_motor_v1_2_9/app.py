...
with tabs[4]:
    ...
    st.caption('Consolidação automática dos vínculos selecionados. Esta é a etapa em que as remunerações e contribuições de todas as fontes da mesma competência são reunidas para calcular o máximo permitido e o valor acima do teto.')

    st.markdown('### Legenda')
    lg1, lg2, lg3 = st.columns(3)
    lg1.success('🟢 **Apuração concluída**')
    lg2.warning('🟡 **Requer análise**')
    lg3.info('📌 **13º — apuração separada**')

    result = apurar(calc_df, dict(TETOS))
    if result.empty:
        st.warning('Sem competências mensais ou 13º para apurar.')
    else:
        # Resumo enxuto, mantendo os detalhes em expansores.
        resumo_ap = []
        for _, rr in result.iterrows():
            ...
            if status == 'OK':
                status_label = '🟢 Concluída' if tipo == 'mensal' else '📌 13º — concluída'
        ...
        st.caption('A ordem de ocupação do teto adotada nesta versão é: Progressiva → 11% → 20%, seguindo a lógica do exemplo prático da planilha-base. O motor é preparado para N vínculos. O 13º é uma apuração própria e não é somado a dezembro.')
        for _, rr in result.iterrows():
            tipo = str(rr.get('tipo_competencia', 'mensal'))
            comp = competencia_br(rr.get('competencia', ''), tipo)
            status = str(rr.get('status', ''))
            ...
            with st.expander(f'{icon} {comp} — Recolhido {fmt(rr.get("soma_contribuicoes_recolhidas",0))} — Acima do teto {fmt(max(excesso,0))}'):
                ...
                if not fontes_rr.empty:
                    ...
                    st.markdown('**Fontes consolidadas na competência**')
