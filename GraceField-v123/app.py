...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.23', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
def gerar_relatorio_pdf(result_df=None,horiz_df=None,declarantes=None,periodo='—',decimos=None,dedup_stats=None):
    buf=BytesIO(); portrait=A4; land=landscape(A4)
    doc=BaseDocTemplate(buf,pagesize=portrait,rightMargin=14*mm,leftMargin=14*mm,topMargin=16*mm,bottomMargin=16*mm,title='Relatório de Apuração — Contribuições Previdenciárias')
    fp=Frame(doc.leftMargin,doc.bottomMargin,portrait[0]-doc.leftMargin-doc.rightMargin,portrait[1]-doc.topMargin-doc.bottomMargin,id='portrait')
    fl=Frame(10*mm,16*mm,land[0]-20*mm,land[1]-28*mm,id='landscape')
    doc.addPageTemplates([PageTemplate(id='P',frames=fp,onPage=_pdf_footer,pagesize=portrait),PageTemplate(id='L',frames=fl,onPage=_pdf_footer,pagesize=land)])
    styles=getSampleStyleSheet(); title=ParagraphStyle('JFT',parent=styles['Title'],fontName='Helvetica-Bold',fontSize=16,leading=19,textColor=colors.HexColor('#17365D'),alignment=TA_CENTER,spaceAfter=7)
    h1=ParagraphStyle('JFH1',parent=styles['Heading2'],fontName='Helvetica-Bold',fontSize=10.5,leading=13,textColor=colors.HexColor('#17365D'),spaceBefore=7,spaceAfter=6); small=ParagraphStyle('JFS',parent=styles['BodyText'],fontSize=7.5,leading=10,textColor=colors.HexColor('#475467'))
    story=[Paragraph('RELATÓRIO TÉCNICO DE APURAÇÃO',title),Paragraph('CONTRIBUIÇÕES PREVIDENCIÁRIAS ACIMA DO TETO',ParagraphStyle('sub',parent=title,fontSize=10.5,leading=13,textColor=colors.HexColor('#475467'))),Spacer(1,4)]
    processo=st.session_state.get('caso_processo','') or '—'; autor=st.session_state.get('caso_autor','') or '—'; reu=st.session_state.get('caso_reu','') or '—'; docid=st.session_state.get('caso_documento_id','') or '—'
    ident=[['IDENTIFICAÇÃO DOS AUTOS',''],['Nº do processo',_pdf_text(processo)],['Autor',_pdf_text(autor)],['Réu',_pdf_text(reu)],['ID do documento analisado',_pdf_text(docid)]]
    ti=_pdf_table(ident,[52*mm,124*mm],8.2,header=False); ti.setStyle(TableStyle([('SPAN',(0,0),(1,0)),('BACKGROUND',(0,0),(1,0),colors.HexColor('#E9EEF5')),('FONTNAME',(0,0),(1,0),'Helvetica-Bold'),('TEXTCOLOR',(0,0),(1,0),colors.HexColor('#17365D'))])); story += [ti,Spacer(1,6)]
    story += [Paragraph('1. Declarantes utilizados na apuração',h1)]
    rows=[['Declarante','CNPJ','Enquadramento','Página(s)']]
    for d in declarantes or []: rows.append([_pdf_text(d.get('nome','')),_pdf_text(d.get('cnpj','')),_pdf_text(d.get('grupo','')),_pdf_text(d.get('paginas',''))])
    if len(rows)==1: rows.append(['Nenhum declarante selecionado','—','—','—'])
    story.append(_pdf_table(rows,[68*mm,43*mm,32*mm,29*mm],7.3))
    if dedup_stats and dedup_stats.get('blocos_duplicados'): story += [Spacer(1,4),Paragraph(f"Auditoria documental: {dedup_stats.get('blocos_duplicados',0)} bloco(s) repetido(s) identificado(s); as ocorrências duplicadas permanecem no RAW e não são contabilizadas novamente.",small)]
    story += [Paragraph('2. Período e critérios utilizados',h1),_pdf_table([['Item','Informação'],['Período utilizado no cálculo',_pdf_text(periodo)],['13º incluídos',_pdf_text(', '.join(f'13º/{a}' for a in (decimos or [])) if decimos else 'Nenhum')],['Base documental','DIRF selecionadas pelo usuário']],[55*mm,121*mm],8)]
    story += [Paragraph('3. Resultado consolidado',h1)]
    if result_df is not None and not result_df.empty:
        cols=['Competência','Tipo','Recolhido','Teto','Máx. progressiva','Máx. 11%','Máx. 20%','Máx. total','Acima do teto','Status']; data=[cols]
        for _,r in result_df.iterrows(): data.append([str(r.get(c,'')) if c in ('Competência','Tipo','Status') else _pdf_money(r.get(c,0)) for c in cols])
        story.append(_pdf_table(data,[21*mm,14*mm,22*mm,21*mm,24*mm,21*mm,21*mm,22*mm,23*mm,29*mm],5.8,align_right_cols=list(range(2,9))))
        tr=float(result_df['Recolhido'].sum()) if 'Recolhido' in result_df else 0; tm=float(result_df['Máx. total'].sum()) if 'Máx. total' in result_df else 0; te=float(result_df['Acima do teto'].sum()) if 'Acima do teto' in result_df else 0
        story += [Spacer(1,5),_pdf_table([['Total das contribuições recolhidas','Total máximo considerado','Total acima do teto'],[_pdf_money(tr,True),_pdf_money(tm,True),_pdf_money(te,True)]],[58*mm,58*mm,58*mm],8.2)]
    else: story.append(_pdf_table([['Resultado'],['Não há apuração disponível para o escopo selecionado.']],[176*mm],8))
    if horiz_df is not None and not horiz_df.empty:
        story += [NextPageTemplate('L'),PageBreak(),Paragraph('4. Demonstração horizontal',h1),Paragraph('Estrutura horizontal dos vínculos utilizados, com Remuneração e Previdência por competência.',small),Spacer(1,3)]
        cols=list(horiz_df.columns); n=len(cols); first=25*mm; remaining=277*mm-first; w=max(12*mm,remaining/max(n-1,1)); data=[cols]
        for _,r in horiz_df.iterrows(): data.append([str(r.iloc[0])]+[_pdf_money(x) for x in r.iloc[1:]])
        story.append(_pdf_table(data,[first]+[w]*(n-1),max(4.0,min(6.8,235/max(n,1))),align_right_cols=list(range(1,n))))
...
st.title('Extrator DIRF + Motor Previdenciário V1.2.22')
...
st.markdown('## Identificação dos autos')
...
with tabs[6]:
    ...
    result_relatorio=result.copy() if 'result' in locals() and isinstance(result,pd.DataFrame) else pd.DataFrame()
